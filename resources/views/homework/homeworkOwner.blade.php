...
<div class="actions d-flex flex-wrap justify-content-center mt-4">
    @if(isset($task->file))
        <a href="{{ route('homework.downloadTask', ['task' => $task->id]) }}" class="btn btn-info"><i class="fas fa-file"></i></a>
    @endif
    ...
    <form action="{{ route('homework.taskEdition', ['task' => $task->id]) }}"
        ...
        <button type="submit" class="btn btn-secondary"><i class="fas fa-pencil-alt"></i></button>
    </form>
